...
export default function Index() {
    ...
    const BuBodyTemplate = (rowData) => {
        return (
            <div><Link href={'/software/detailedview/businessunit'} className='flex items-center space-x-2'><i className='red-tsg-plus'></i><span>Business Unit</span></Link></div>
        );
    };
    ...
    const myCustomHeaderFilter = (columnName = '', isFilterApplicable = false) => {
      return <div className="flex justify-between flex-row-reverse gap-2">  
        <div className='dark:text-[#898A92] text-[#344054]'>{columnName}</div>  
        {isFilterApplicable &&  
          <div>  
            <Link href={"/software/detailedview"} className='dark:text-[#898A92] text-[#344054]'><i className="red-tsg-back-rounded text-base"></i></Link>  
          </div>
  ...
  return (
    ...
    <div className='bg-white dark:bg-[#14161A] border border-[#C6CBD2] dark:border-[#14161A] rounded-tl-lg rounded-tr-lg px-5 xl:px-[1.250vw] py-2 flex justify-between items-center'>
        <div className='flex gap-3 items-center'>
            ...
            <div className='text-[#888888] text-sm space-x-2 font-normal'>
                ...
                <span href={''}><i className='red-tsg-right-arrow text-[10px] text-[#888888]'></i></span>
                <span href={''}>Tableu</span>
            </div>
        </div>
        <div className='flex items-center gap-2'>
            ...
            <div><Link href={'/software/detailedview/quarterly/tableu'} className='text-[#344054] dark:text-[#CACED1] font-medium text-sm xl:text-[0.729vw] rounded-lg xl:px-4 px-[1.042vw] py-2 xl:py-[0.521vw] bg-white dark:bg-[#272F31] border border-[#C6CBD2] dark:border-[rgba(221,232,237,0.10)] inline-block'>Quarterly</Link></div>
    ...
    <DataTable value={sales} footerColumnGroup={footerGroup} scrollable paginator rows={8} rowsPerPageOptions={[5, 10, 25, 50]} paginatorTemplate="PrevPageLink PageLinks NextPageLink" className='custpaginator custIcons custmBtnTable custTable' tableStyle={{ minWidth: '100%' }} >
                <Column field="product" headerClassName='header-filter' header={myCustomHeaderFilter('Tableu', true)} body={BuBodyTemplate} style={{width:"13rem"}}  />
